...
import { useUniversityFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['course-coordinator', 'business-owner', 'faculty-member', 'student'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useUniversityFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Universities',
      path: isTenantUser ? `/universities/view/${data?.id}` : '/universities',
      entity: 'university',
